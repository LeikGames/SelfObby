...
public static class SpringUtils
{
    ...
    ) // motion parameters to use
    {
        float oldPos = pPos - equilibriumPos; // update in equilibrium relative space
        float oldVel = pVel;

        (pPos) =
            oldPos * springParams.m_posPosCoef
            + oldVel * springParams.m_posVelCoef
            + equilibriumPos;
        (pVel) = oldPos * springParams.m_velPosCoef + oldVel * springParams.m_velVelCoef;
    }

    /// <summary>
    /// Does CalcSpringMotionParams as well a UpdateDampedSpringMotions
    /// without having to handle a tDampedSpringMotionParams in between.
    /// </summary>
    public static void DoHarmonicSpringMotion(
        ref float position,
        ref float velocity,
    ...
    {
        tDampedSpringMotionParams motionParams = new();

        CalcDampedSpringMotionParams(ref motionParams, deltaTime, angularFrequency, dampingRatio);

        UpdateDampedSpringMotion(ref position, ref velocity, equilibriumPosition, in motionParams);
    }

    public static void DoHarmonicSpringMotion(
        ref Vector2 position,
        ref Vector2 velocity,
        Vector2 equilibriumPosition,
        float deltaTime,
        float angularFrequency,
        float dampingRatio
    )
    {
        ...
        UpdateDampedSpringMotion(
            ref position.x,
            ref velocity.x,
            equilibriumPosition.x,
            in motionParams
        );
        UpdateDampedSpringMotion(
            ref position.y,
            ref velocity.y,
            equilibriumPosition.y,
            in motionParams
        );
    }
